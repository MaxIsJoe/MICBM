...
func set_element_values():
	%master_slider.value = db_to_linear( OptionsManager.options["master_volume"] )
	%music_slider.value = db_to_linear( OptionsManager.options["music_volume"] )
	%sfx_slider.value = db_to_linear( OptionsManager.options["sound_volume"] )
